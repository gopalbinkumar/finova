...
export function AddDebtModal({ open, onClose, defaultType = 'debt' }) {
    ...
    const validate = () => {
        ...
        if (!form.borrower.trim())
            e.borrower = isDebt ? 'Lender name required' : 'Borrower name required';
    ...
    const daysLeft = form.dueDate
        ...
        : null;
    return (<Modal open={open} onClose={onClose} title={isDebt ? 'Record a Debt' : 'Record Receivable'} subtitle={isDebt ? 'Money you owe to someone' : 'Money owed to you'} icon={<CreditCard size={20}/>} iconColor={isDebt ? '#EF4444' : '#2563EB'} footer={done
      ...
      <div className="space-y-4">
        ...
        <FormField label="Record Type" required>
          <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-muted">
            {['debt', 'receivable'].map(t => (<button key={t} type="button" onClick={() => set('type', t)} className={`py-2.5 rounded-lg text-sm font-semibold capitalize transition-all ${form.type === t
                ? t === 'debt'
        ...
        {/* Borrower/Lender */}
        <FormField label={isDebt ? 'Lender / Creditor' : 'Borrower Name'} required error={errors.borrower}>
          <Input placeholder={isDebt ? 'e.g. Student Loan, Bank' : 'e.g. Marcus Chen'} value={form.borrower} onChange={e => set('borrower', e.target.value)} error={!!errors.borrower} autoFocus/>
        </FormField>

        {/* Total amount + Remaining */}
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Total Amount" required error={errors.amount}>
            <Input type="number" step="0.01" min="1" placeholder="1,000" value={form.amount} onChange={e => set('amount', e.target.value)} error={!!errors.amount} leftDecor="$"/>
          </FormField>
          <FormField label="Remaining Balance" hint="Leave blank if none paid">
            <Input type="number" step="0.01" min="0" placeholder="Same as total" value={form.remaining} onChange={e => set('remaining', e.target.value)} leftDecor="$"/>
          </FormField>
        </div>
        ...
        <FormField label="Notes" hint="Purpose of the debt">
          <textarea className="finova-input w-full text-sm resize-none" rows={2} placeholder={isDebt ? 'e.g. Car loan from bank' : 'e.g. Trip expense loan'} value={form.notes} onChange={e => set('notes', e.target.value)}/>
        </FormField>

        {/* Preview */}
        {amount > 0 && (<div className="p-4 rounded-xl border space-y-3" style={{
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              {daysLeft !== null && (<span className={daysLeft < 14 ? 'text-red-500 font-semibold' : ''}>
