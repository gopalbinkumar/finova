...
export function AddBudgetModal({ open, onClose }) {
    ...
    const validate = () => {
        ...
        if (!form.limit || Number(form.limit) <= 0)
            e.limit = 'Enter a valid budget limit';
        return e;
    };
    ...
    return (<Modal open={open} onClose={onClose} title="Create Budget" subtitle="Set a spending limit for a category" icon={<PieChart size={20}/>} iconColor="#8B5CF6" footer={done
      ...
      <div className="space-y-4">
        ...
        <FormField label="Category" required error={errors.category}>
          <Select value={form.category} onChange={e => set('category', e.target.value)} options={expenseCategories} placeholder="Select a category..." error={!!errors.category}/>
        </FormField>

        {/* Period */}
        <FormField label="Budget Period" required>
          <Select value={form.period} onChange={e => set('period', e.target.value)} options={PERIOD_OPTIONS}/>
        </FormField>

        {/* Budget Limit */}
        <FormField label="Monthly Limit" required error={errors.limit}>
          <Input type="number" step="1" min="1" placeholder="500" value={form.limit} onChange={e => set('limit', e.target.value)} error={!!errors.limit} leftDecor="$"/>
        </FormField>
        ...
        <FormField label={`Alert me at ${form.alertAt}% — ${alertAmount > 0 ? `$${alertAmount.toFixed(0)}` : '--'}`} hint="Get notified before you hit the limit">
          <div className="space-y-2">
            <input type="range" min="50" max="100" step="5" value={form.alertAt} onChange={e => set('alertAt', e.target.value)} className="w-full accent-primary-500"/>
        ...
        <FormField label="Notes" hint="Optional">
          <textarea className="finova-input w-full text-sm resize-none" rows={2} placeholder="e.g. Cut back on dining out" value={form.notes} onChange={e => set('notes', e.target.value)}/>
        </FormField>

        {/* Summary card */}
        {form.category && limit > 0 && (<div className="p-4 rounded-xl bg-muted/40 border border-border space-y-3">
