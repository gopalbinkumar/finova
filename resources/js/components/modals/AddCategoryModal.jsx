...
export function AddCategoryModal({ open, onClose, defaultType = 'expense' }) {
    ...
    return (<Modal open={open} onClose={onClose} title="New Category" subtitle="Organize your transactions with categories" icon={<Tag size={20}/>} iconColor="#8B5CF6" footer={done
      ...
      <div className="space-y-5">
        ...
        <FormField label="Category Type" required>
          <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-muted">
            {['expense', 'income'].map(t => (<button key={t} type="button" onClick={() => set('type', t)} className={`py-2.5 rounded-lg text-sm font-semibold capitalize transition-all ${form.type === t
                ? t === 'expense'
        ...
        <FormField label="Category Name" required error={errors.name}>
          <Input placeholder="e.g. Coffee & Drinks" value={form.name} onChange={e => set('name', e.target.value)} error={!!errors.name} autoFocus/>
        </FormField>
        ...
        <FormField label="Color">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <input type="color" value={form.color} onChange={e => set('color', e.target.value)} className="w-8 h-8 rounded-lg cursor-pointer border border-border"/>
          </div>
        </FormField>
        ...
        <div className="flex items-center gap-4 p-4 rounded-xl border border-border bg-muted/20">
          ...
          <div>
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full capitalize" style={{ background: `${form.color}20`, color: form.color }}>
              {form.type}
            </span>
          </div>
        </div>
