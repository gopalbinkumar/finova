...
export function StatCard({ title, value, subtitle, icon, trend, accentColor = '#2563EB', className = '' }) {
    const trendPositive = (trend?.value ?? 0) >= 0;
    return (<div className={`finova-card group cursor-default ${className}`}>
      <div className="flex items-start justify-between mb-4">
        <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110" style={{ background: `${accentColor}20`, color: accentColor }}>
          {icon}
        </div>
        {trend && (<div className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${trendPositive ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'}`}>
            {trendPositive ? <TrendingUp size={12}/> : trend.value === 0 ? <Minus size={12}/> : <TrendingDown size={12}/>}
...
export function ProgressBar({ value, max = 100, color = '#2563EB', showLabel = true, height = 8, className = '' }) {
    ...
    return (<div className={`w-full ${className}`}>
      ...
      {showLabel && (<div className="flex justify-between mt-1">
          ...
          <span className={`text-xs font-medium ${overBudget ? 'text-red-500' : warning ? 'text-amber-500' : 'text-muted-foreground'}`}>
...
const badgeStyles = {
    success: 'bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300',
...
export function Badge({ children, variant = 'neutral' }) {
    return (<span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${badgeStyles[variant]}`}>
      {children}
    </span>);
}
// ─── Empty State ───────────────────────────────────────────────────────────────
