...
interface Props { open: boolean; onClose: () => void; defaultType?: 'income' | 'expense' | 'transfer' }
...
export function AddTransactionModal({ open, onClose, defaultType = 'expense' }: Props) {
  ...
  const validate = () => {
    ...
    if (form.type !== 'transfer' && !form.category) e.category = 'Please select a category'
    if (form.type === 'transfer' && !form.toAccount) e.toAccount = 'Please select destination account'
    return e
  }
    ...
    >
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2 p-1 rounded-xl bg-muted">
            {(['expense', 'income', 'transfer'] as const).map(t => (
              <button
                key={t}
                type="button"
                onClick={() => { set('type', t); setForm(f => ({ ...f, category: '', toAccount: '' })) }}
                className={`py-2.5 rounded-lg text-sm font-semibold capitalize transition-all ${
                  form.type === t ? 'text-white shadow' : 'text-muted-foreground hover:text-foreground'
                }`}
                style={form.type === t ? { background: typeColors[t] } : {}}
              >
        ...
        <FormField label="Amount" required error={errors.amount}>
          <div className="relative">
            ...
            <input
              ...
              className={`finova-input w-full pl-10 text-2xl font-bold py-4 ${errors.amount ? 'border-red-500' : ''}`}
        ...
        <div className={`grid gap-4 ${form.type === 'transfer' ? 'grid-cols-2' : 'grid-cols-1'}`}>
          ...
          {form.type === 'transfer' && (
            <FormField label="To Account" required error={errors.toAccount}>
              <Select
                value={form.toAccount}
                onChange={e => set('toAccount', e.target.value)}
                options={accountOptions.filter(a => a.value !== form.account)}
        ...
        {form.amount && Number(form.amount) > 0 && (
          <div
            ...
            style={{ borderColor: `${typeColor}30`, background: `${typeColor}08` }}
          >
            <div
