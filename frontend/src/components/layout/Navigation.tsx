...
import {
  LayoutDashboard, Wallet, Tag, ArrowLeftRight, PieChart,
  Target, TrendingUp, CreditCard, BarChart2, Bell, Settings,
  LogOut, X, ChevronLeft, ChevronRight, Menu,
} from 'lucide-react'
...
export function Sidebar({ collapsed, onCollapse, onMobileClose, mobile = false }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center justify-between px-4 py-5 border-b border-white/10 flex-shrink-0`}>
        ...
        {mobile ? (
          <button onClick={onMobileClose} className="text-white/60 hover:text-white p-1 rounded transition-colors">
            <X size={20} />
          </button>
        ) : (
          <button
      ...
      <div className="border-t border-white/10 px-2 py-3 space-y-1 flex-shrink-0">
        <NavLink
          to="/profile"
          onClick={onMobileClose}
          className={({ isActive }) => `sidebar-item ${isActive ? 'active' : ''} ${collapsed && !mobile ? 'justify-center px-2' : ''}`}
        ...
        {(collapsed && !mobile) && (
          <button onClick={handleLogout} className="sidebar-item justify-center px-2 w-full" title="Logout">
            <LogOut size={18} />
          </button>
        )}
      </div>
...
export function MobileSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-black/60 z-40 transition-opacity duration-300 lg:hidden ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />
      {/* Drawer */}
      <div
        className={`fixed inset-y-0 left-0 z-50 transition-transform duration-300 lg:hidden ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
    ...
    <header className="sticky top-0 z-30 bg-card/80 backdrop-blur-md border-b border-border h-16 flex items-center px-4 lg:px-6 gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-2 pl-2 rounded-lg hover:bg-muted transition-colors"
        >
