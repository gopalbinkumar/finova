    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <div className="flex items-start gap-4 px-6 pt-6 pb-4 border-b border-border flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-muted text-muted-foreground hover:text-foreground transition-colors flex-shrink-0 -mt-1 -mr-1"
            aria-label="Close"
          >
            <X size={20} />
          </button>
...
export function Input({ error, leftDecor, rightDecor, className = '', ...props }: InputProps) {
  return (
...
export function Select({ error, options, placeholder, className = '', ...props }: SelectProps) {
  return (
    <select
      className={`finova-input w-full text-sm appearance-none cursor-pointer ${error ? 'border-red-500' : ''} ${className}`}
      {...props}
    >
...
export function Textarea({ error, className = '', ...props }: TextareaProps) {
  return (
    <textarea
      className={`finova-input w-full text-sm resize-none ${error ? 'border-red-500' : ''} ${className}`}
      rows={3}
      {...props}
    />
  )
...
export function ModalFooter({ onCancel, onSubmit, submitLabel = 'Save', cancelLabel = 'Cancel', loading = false, danger = false }: ModalFooterProps) {
  return (
    <div className="flex items-center justify-end gap-3">
      ...
      <button
        ...
        onClick={onSubmit}
        disabled={loading}
        className={`${danger ? 'btn-danger' : 'btn-primary'} px-5 py-2 text-sm flex items-center gap-2`}
      >
        {loading && (
