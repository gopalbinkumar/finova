    ...
    <div className="space-y-6 animate-in max-w-2xl mx-auto">
      ...
      <div className="finova-card flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
            ...
            {(['all', 'unread'] as const).map(f => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold capitalize transition-colors ${
                  filter === f ? 'bg-primary-500 text-white' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                }`}
              >
                {f}
              </button>
          ...
          {unread > 0 && (
            <button onClick={markAll} className="btn-secondary text-sm px-3 py-1.5 flex items-center gap-1.5">
              <CheckCheck size={14} />
              Mark all read
            </button>
          )}
        ...
        {visible.length === 0 ? (
          ...
          <div>
            {visible.map((notif, i) => (
              <div
                key={notif.id}
                className={`flex items-start gap-4 p-5 border-b border-border last:border-0 group transition-colors cursor-pointer hover:bg-muted/30
                  ${!notif.read ? 'bg-primary-50/30 dark:bg-primary-900/5' : ''}`}
                ...
                <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                  {!notif.read && (
                    <button onClick={e => { e.stopPropagation(); markRead(notif.id) }}
                      className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Mark read">
                      <Check size={14} />
                    </button>
                  )}
                  <button onClick={e => { e.stopPropagation(); dismiss(notif.id) }}
                    className="p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-muted-foreground hover:text-red-500 transition-colors" title="Dismiss">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
