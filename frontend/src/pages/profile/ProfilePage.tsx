...
import { z } from 'zod'
import {
  User, Mail, Phone, Globe, Clock, Palette,
  Camera, Trash2, Eye, EyeOff, Lock, CheckCircle2,
  Settings
} from 'lucide-react'
import {
  useMe,
        ...
        <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
          ...
          {user?.avatar_url && (
            <button
              ...
              className="btn-danger text-sm px-4 py-1.5 flex items-center gap-1.5"
...
function ProfileForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      {success && (
        <div className="bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 text-primary-700 dark:text-primary-400 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input id="profile-name" type="text" placeholder="John Doe"
              className={`finova-input pl-10 ${errors.name ? 'border-destructive' : ''}`}
              {...register('name')} />
          </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input id="profile-phone" type="tel" placeholder="+1 555 000 0000"
              className="finova-input pl-10" {...register('phone')} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <select id="profile-currency"
              className="finova-input pl-10 appearance-none cursor-pointer"
              {...register('currency')}>
...
function ChangePasswordForm() {
  ...
  const onSubmit = async (data: PasswordFormData) => {
    ...
    setSuccess(false)
    try {
      await changePassword.mutateAsync(data as ChangePasswordData)
      setSuccess(true)
      reset()
      setTimeout(() => setSuccess(false), 4000)
    } catch (err) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input id="cp-current" type={showCurrent ? 'text' : 'password'} placeholder="Your current password"
            className={`finova-input pl-10 pr-10 ${errors.current_password ? 'border-destructive' : ''}`}
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <div className="relative">
            <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <input id="cp-new" type={showNew ? 'text' : 'password'} placeholder="Min. 8 characters"
              className={`finova-input pl-10 pr-10 ${errors.password ? 'border-destructive' : ''}`}
              {...register('password')} />
        ...
        <div>
          ...
          <div className="relative">
            <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <input id="cp-confirm" type={showConfirm ? 'text' : 'password'} placeholder="Repeat new password"
              className={`finova-input pl-10 pr-10 ${errors.password_confirmation ? 'border-destructive' : ''}`}
