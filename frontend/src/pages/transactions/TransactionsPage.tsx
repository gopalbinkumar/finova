...
export function TransactionsPage() {
  ...
  const [defaultTxType, setDefaultTxType] = useState<'income' | 'expense' | 'transfer'>('expense')
  ...
  const txBadge = (type: string) => {
    ...
    if (type === 'transfer') return <Badge variant="info">Transfer</Badge>
      ...
      <div className="space-y-6 animate-in">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={s.label} className={`finova-card text-center ${s.bg}`}>
                  ...
                  <tr key={tx.id} className="cursor-pointer">
                    <td>
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                          tx.type === 'income'   ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-500' :
                    ...
                    <td className="text-right">
                      <span className={`font-bold text-sm ${
                        tx.type === 'income'   ? 'text-primary-500' :
