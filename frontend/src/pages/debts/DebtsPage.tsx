...
import { Plus, AlertCircle, CheckCircle2, Clock, Pencil, Trash2 } from 'lucide-react'
...
export function DebtsPage() {
  ...
  const debts       = mockDebts.filter(d => d.type === 'debt'       && !d.isPaid)
  const receivables = mockDebts.filter(d => d.type === 'receivable' && !d.isPaid)
  const paid        = mockDebts.filter(d => d.isPaid)

  const totalDebt   = debts.reduce((s, d) => s + d.remaining, 0)
  const totalReceiv = receivables.reduce((s, d) => s + d.remaining, 0)

  const openModal = (type: 'debt' | 'receivable') => {
    setModalType(type)
    setShowModal(true)
  }
          ...
          <div className="finova-card">
            ...
            <div className="space-y-4">
              {debts.map(debt => {
                ...
                const pct  = (paidAmount / debt.amount) * 100
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button className="p-1.5 rounded hover:bg-red-100 text-muted-foreground hover:text-red-500">
                          <Trash2 size={12} />
                        </button>
                      </div>
                    </div>
