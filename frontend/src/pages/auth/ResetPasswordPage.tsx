...
export function ResetPasswordPage() {
  ...
  if (!token || !email) {
    return (
      <div className="finova-card auth-glow text-center">
  ...
  return (
    <div className="finova-card auth-glow animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        <div>
          <label htmlFor="reset-password" className="finova-label">New password</label>
        ...
        <div>
          <label htmlFor="reset-confirm" className="finova-label">Confirm new password</label>
          <div className="relative">
            <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <input
              id="reset-confirm"
          ...
          {errors.password_confirmation && (
            <p className="text-destructive text-xs mt-1">{errors.password_confirmation.message}</p>
          )}
        </div>
